const employees = [
      ...
      { id: 4, name: 'Adam Antonelly', age: 25, department: 'HR', salary: 70000, specialization: 'Javascript' },
...
// function to display the specialization
function findEmployeeByspec(employeeSpec) {
  const foundEmployee = employees.find(employee => employee.specialization === employeeSpec);
      ...
      document.getElementById('employeesDetails').innerHTML =`<p>${foundEmployee.id}: ${foundEmployee.name}  - ${foundEmployee.department} - $${foundEmployee.salary} - ${foundEmployee.specialization}</p>`;
        ...
        document.getElementById('employeesDetails').innerHTML = 'no employee has been found with this Specilaization';
